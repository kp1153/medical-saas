...
export default async function Profit() {
  ...
  for (const item of sold) {
    ...
    if (!medicineMap[name])
      medicineMap[name] = { name, revenue: 0, cost: 0, qty: 0 };
  ...
  const medicines = Object.values(medicineMap)
    .map((m) => ({
      ...m,
      profit: m.revenue - m.cost,
      margin:
        m.revenue > 0
          ? (((m.revenue - m.cost) / m.revenue) * 100).toFixed(1)
          : 0,
    }))
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
        <div className="grid grid-cols-3 gap-4">
          ...
          <div
            className={`bg-white rounded-xl shadow p-4 border-l-4 ${totalProfit >= 0 ? "border-blue-500" : "border-red-600"}`}
          >
            <p className="text-xs text-gray-500 uppercase font-semibold">
              Net Profit
            </p>
            <p
              className={`text-2xl font-bold mt-1 ${totalProfit >= 0 ? "text-blue-700" : "text-red-700"}`}
            >
            ...
            <tbody>
              ...
              {medicines.map((m, i) => (
                <tr key={i} className="border-b hover:bg-gray-50">
                  ...
                  <td
                    className={`px-4 py-3 font-bold ${m.profit >= 0 ? "text-blue-700" : "text-red-700"}`}
                  >
                    ₹{m.profit.toFixed(0)}
                  </td>
