...
export default async function AllBills() {
  ...
  const sorted = allSales.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
        ...
        <div className="bg-white rounded-xl shadow overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-blue-900 text-white">
              <tr>
                {["Bill No", "Patient", "Phone", "Amount", "Payment", "Date", "Action"].map((h) => (
            ...
            <tbody>
              {sorted.length === 0 && (
                <tr><td colSpan={7} className="text-center py-8 text-gray-400">No bills yet</td></tr>
              )}
              {sorted.map((b) => (
                <tr key={b.id} className="border-b hover:bg-gray-50">
                  <td className="px-4 py-3 font-medium text-blue-700">{b.billNo}</td>
                  <td className="px-4 py-3 text-gray-800">{b.patientName}</td>
                  <td className="px-4 py-3 text-gray-500">{b.patientPhone || "—"}</td>
                  <td className="px-4 py-3 font-semibold text-gray-800">₹{b.netAmount.toFixed(2)}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold capitalize ${
                      b.paymentType === "Credit" ? "bg-red-100 text-red-700" :
                      b.paymentType === "upi" ? "bg-blue-100 text-blue-700" :
                      "bg-green-100 text-green-700"
                    }`}>{b.paymentType}</span>
                  </td>
                  <td className="px-4 py-3 text-gray-500">{new Date(b.createdAt).toLocaleDateString("en-IN")}</td>
                  <td className="px-4 py-3">
                    <Link href={`/sales/${b.id}`} className="text-blue-600 hover:underline text-xs">View</Link>
                  </td>
                </tr>
              ))}
            </tbody>
