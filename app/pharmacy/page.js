...
export default async function Pharmacy() {
  const all = await db.select().from(prescriptions).where(eq(prescriptions.sentToPharmacy, 1));

  const queue = await Promise.all(
    all.map(async (rx) => {
      const patient = await db.select().from(patients).where(eq(patients.id, rx.patientId)).limit(1);
