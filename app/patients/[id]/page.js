...
import { patients, prescriptions, sales, labReports } from "@/lib/schema";
...
export default async function PatientHistory({ params }) {
  ...
  const [patientArr, allPrescriptions, allSales, allLabReports] =
    await Promise.all([
      db.select().from(patients).where(eq(patients.id, pid)).limit(1),
      db
        .select()
        .from(prescriptions)
        .where(eq(prescriptions.patientId, pid))
        .orderBy(desc(prescriptions.createdAt)),
      db
        .select()
        .from(sales)
        .where(eq(sales.patientId, pid))
        .orderBy(desc(sales.createdAt)),
      db
        .select()
        .from(labReports)
        .where(eq(labReports.patientId, pid))
          ...
          {allPrescriptions.length === 0 ? (
            ...
            <div className="divide-y">
              {allPrescriptions.map((rx) => {
                ...
                const rxReports = allLabReports.filter(
                  (r) => r.prescriptionId === rx.id,
                );
                return (
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {meds.map((m, i) => (
                            <span
                              key={i}
                              className="bg-blue-50 text-blue-800 px-2 py-1 rounded-lg text-xs"
                            >
