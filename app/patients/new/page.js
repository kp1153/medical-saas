...
export default function NewPatient() {
  ...
  const [form, setForm] = useState({
    name: "", phone: "", address: "", age: "", gender: "", complaint: "",
  });
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!form.name.trim()) return alert("नाम जरूरी है");
    setSaving(true);
    ...
    setSaving(false);
    if (data.success) router.push(`/patients/${data.id}`);
  }

  return (
    <div className="min-h-screen bg-gray-100">
      <header className="bg-blue-900 text-white px-6 py-3 flex items-center gap-4 shadow-md">
        <Link href="/patients" className="text-blue-300 hover:text-white text-sm">
          ← Back
        </Link>
