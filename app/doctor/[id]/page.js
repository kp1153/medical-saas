...
const TEST_CATEGORIES = {
  "🩸 Blood Tests": ["CBC","Blood Sugar Fasting","Blood Sugar PP","HbA1c","Lipid Profile","LFT","KFT","Thyroid TSH","Thyroid T3/T4","Uric Acid","ESR","CRP","Serum Calcium","Vitamin D","Vitamin B12","Iron Studies","PT/INR","Dengue NS1","Malaria","Typhoid Widal"],
  "🧪 Urine / Stool": ["Urine R/M","Urine Culture","24hr Urine Protein","Stool R/M","Stool Culture"],
  "📷 X-Ray": ["X-Ray Chest PA","X-Ray KUB","X-Ray Spine LS","X-Ray Spine CS","X-Ray Hand","X-Ray Knee","X-Ray Pelvis","X-Ray Shoulder","X-Ray Ankle","X-Ray PNS"],
  "🔊 Ultrasound": ["USG Abdomen","USG Pelvis","USG Abdomen + Pelvis","USG Thyroid","USG Obstetric","USG KUB","USG Breast","USG Scrotum"],
  "🧲 MRI": ["MRI Brain","MRI Spine LS","MRI Spine CS","MRI Knee","MRI Shoulder","MRI Abdomen","MRI Pelvis","MRI Wrist"],
  "🔬 CT Scan": ["CT Brain","CT Chest","CT Abdomen","CT KUB","CT Spine LS","CT PNS","CT Angiography","CT Pelvis"],
  "❤️ Cardiac": ["ECG","Echo 2D","TMT","Holter 24hr"],
  "👁️ Other": ["Biopsy","Endoscopy","Colonoscopy","PFT","EEG","EMG/NCV","Bone Density (DEXA)"],
};

export default function DoctorExamine() {
  const router = useRouter();
  const { id: patientId } = useParams(); // ← fix: एक बार
  ...
  const [selectedCategory, setSelectedCategory] = useState(Object.keys(TEST_CATEGORIES)[0]);
  ...
  function addMedicine(med) {
    if (items.find((i) => i.medicineId === med.id)) { setSearch(""); setSuggestions([]); return; }
    setItems([...items, {
      medicineId: med.id, medicineName: med.name,
      generic: med.generic || "", dose: "", duration: "", instruction: "After food",
    }]);
    setSearch(""); setSuggestions([]);
  }

  function updateItem(idx, field, value) {
    const updated = [...items]; updated[idx][field] = value; setItems(updated);
  }
  ...
  async function handleSubmit() {
    if (!items.length && !selectedTests.length) return alert("Add at least one medicine or test");
    ...
    const res = await fetch("/api/prescriptions", {
      ...
      body: JSON.stringify({ patientId: patient.id, diagnosis, notes, medicines: items, tests: selectedTests }),
    });
    ...
    for (const test of selectedTests) {
      await fetch("/api/lab-reports", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ patientId: patient.id, prescriptionId: data.id, category: test.category, testName: test.name }),
      });
    }
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
        <div className="bg-white rounded-xl shadow p-4 grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
          ...
          <div><span className="text-gray-500">Complaint:</span> <span className="font-medium text-red-600">{patient.complaint || "—"}</span></div>
        </div>
              ...
              <div className="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-lg shadow-lg z-10 mt-1">
                {suggestions.map((m) => (
                  <div key={m.id} onClick={() => addMedicine(m)}
                    ...
                    <span className="float-right text-xs text-gray-400">Stock: {m.stock}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {items.length > 0 && (
          <div className="bg-white rounded-xl shadow overflow-x-auto">
            <div className="px-4 py-3 border-b font-semibold text-gray-700 text-sm">💊 Medicines</div>
                  ...
                  <tr key={idx} className="border-b">
                    ...
                    <td className="px-3 py-2">
                      <input value={item.dose} onChange={(e) => updateItem(idx, "dose", e.target.value)}
                        className="w-24 border border-gray-300 rounded px-2 py-1 text-sm" placeholder="1-0-1" />
                    </td>
                    <td className="px-3 py-2">
                      <input value={item.duration} onChange={(e) => updateItem(idx, "duration", e.target.value)}
                        className="w-24 border border-gray-300 rounded px-2 py-1 text-sm" placeholder="5 days" />
                    </td>
                    <td className="px-3 py-2">
                      <select value={item.instruction} onChange={(e) => updateItem(idx, "instruction", e.target.value)}
        ...
        <div className="bg-white rounded-xl shadow p-4">
          <p className="font-semibold text-gray-700 text-sm mb-3">🧪 Investigations / Tests</p>
              ...
              return (
                <button key={test} onClick={() => toggleTest(test)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition border ${
                    isSelected ? "bg-green-100 border-green-500 text-green-700" : "bg-white border-gray-300 text-gray-600 hover:border-green-400"
                  }`}>
                  {isSelected ? "✓ " : ""}{test}
                </button>
              );
          ...
          {selectedTests.length > 0 && (
            <div className="mt-3 pt-3 border-t">
              <p className="text-xs text-gray-500 mb-2 font-semibold">Selected Tests:</p>
              <div className="flex flex-wrap gap-2">
                {selectedTests.map((t) => (
                  <span key={t.name} className="bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1">
                    {t.name}
                    <button onClick={() => toggleTest(t.name)} className="text-green-600 hover:text-red-500 font-bold">×</button>
                  </span>
                ))}
              </div>
            </div>
        ...
        <PrintPrescription patient={patient} diagnosis={diagnosis} notes={notes} items={items} selectedTests={selectedTests} />
